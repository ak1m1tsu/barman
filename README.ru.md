...
notifications:
  error_webhook_url: ""    # Discord webhook URL для оповещений об ошибках (оставить пустым для отключения)
  activity_webhook_url: "" # Discord webhook URL для событий пользователей (оставить пустым для отключения)
```
...
Можно настроить два опциональных Discord webhook URL:

| Поле | Что отправляется |
|---|---|
| `error_webhook_url` | Все записи уровня `Error` / `Fatal` / `Panic` |
| `activity_webhook_url` | Действия пользователей: слэш-команды, префиксные реакции, входы участников, изменение префикса, изменение авто-роли |

Оставьте поле пустым, чтобы отключить соответствующий вебхук.
